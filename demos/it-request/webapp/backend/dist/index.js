...
app.get('/api/user-task/:wfRunId/:userTaskGuid', async (req, res) => {
    ...
    if (!utdId)
        return res.status(500).json({ error: 'userTaskDefId missing on UserTaskRun' });
    const userTaskDef = await client.getUserTaskDef(utdId);
    res.json({ userTaskRun, userTaskDef });
});
function toVar(value, type) {
    switch (type) {
...
app.post('/api/admin/delete-wfrun/:wfRunId', async (req, res) => {
    ...
    if (!wfRunId)
        return res.status(400).json({ error: 'wfRunId is required' });
...
app.post('/api/admin/delete-wfruns/:wfSpecName', async (req, res) => {
    ...
    if (!wfSpecName)
        return res.status(400).json({ error: 'wfSpecName is required' });
    let bookmark = undefined;
    let totalDeleted = 0;
    // Paginate through all WfRuns for the given spec and delete them
    for (;;) {
        const list = await client.searchWfRun({ wfSpecName, limit: 100, ...(bookmark ? { bookmark } : {}) });
        const results = list?.results || [];
        for (const item of results) {
            const idStr = typeof item.id === 'string'
                ? item.id
                : item.id?.id;
            if (idStr) {
