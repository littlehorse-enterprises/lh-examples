...
from process_data import get_process_data_workflow, load_pdf, chunk_text, embed_and_store, generate, store_summary, retrieve
from chat_with_pdf import chat_workflow, invoke_chat
...
async def chat():
    ...
    while True:
        ...
        #await workflow?
        client.RunWf(RunWfRequest(
                wf_spec_name="chat-with-llm",
                variables={"user-question": VariableValue(str=user_input)}
        ))
         
        
async def start_and_process():
    await asyncio.gather(start_workers(), process_data(), chat())
